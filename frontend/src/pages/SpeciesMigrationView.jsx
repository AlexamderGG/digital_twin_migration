  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 transition-colors duration-200">
      <div className="w-full lg:w-96 p-6 bg-white dark:bg-slate-800 shadow-md z-10 flex flex-col gap-5 overflow-y-auto border-r border-transparent dark:border-slate-700 transition-colors duration-200">
        <div>
          <h2 className="text-xl font-bold text-emerald-800 dark:text-emerald-500">{t('migration.title')}</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">{t('migration.subtitle')}</p>
        </div>

        <div>
          <label className="block text-sm font-semibold mb-1 dark:text-slate-300">{t('migration.species_label')}</label>
          <select 
          ...
          >
            {especies.map(esp => (
              <option key={esp.id_especie} value={esp.id_especie}>
                {esp.nombre_cientifico} ({esp.nombre_comun || t('migration.no_common_name')})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-xl transition-colors">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-semibold dark:text-slate-200">{t('migration.temp_label')}</span>
        ...
        >
          {loading ? t('migration.loading') : t('migration.simulate_btn')}
        </button>
        ...
        {error && (
          <div className="mt-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 p-3 rounded-lg text-sm border border-rose-200 dark:border-rose-800">
            <strong>{t('migration.attention')}</strong> {error}
          </div>
        )}
          ...
          <div className="flex flex-col gap-3 mt-2 border-t dark:border-slate-700 pt-4 text-sm animate-fadeIn">
            ...
            <div className="grid grid-cols-2 gap-2 text-center">
              <div className="p-2.5 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                <span className="text-xs text-slate-500 dark:text-slate-400 block">{t('migration.displacement')}</span>
                <span className="font-bold text-slate-800 dark:text-slate-200">{predictionData.metricas.distancia_km} km</span>
              </div>
              <div className="p-2.5 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                <span className="text-xs text-slate-500 dark:text-slate-400 block">{t('migration.habitat_change')}</span>
          ...
          {predictionData && (
            <React.Fragment>
              ...
              {predictionData.centroide_actual && (
                <Marker position={[predictionData.centroide_actual.lat, predictionData.centroide_actual.lon]} icon={createCentroidIcon('#2563eb')}><Popup>{t('migration.current')}</Popup></Marker>
              )}
              {predictionData.centroide_futuro && (
                <Marker position={[predictionData.centroide_futuro.lat, predictionData.centroide_futuro.lon]} icon={createCentroidIcon('#dc2626')}><Popup>{t('migration.future')}</Popup></Marker>
              )}
